...
result_dir = base_dir + "/results/sqlite_cachestat_doublecaching4"
bpf = base_dir + "/bpf/cachestat/leveldb_cachestat.py"

sqlite_basic_command = "cgexec -g memory:{} {} -r --cache_size={} --read_ratio={} " \
                + "| tee {}"
preparation_command = "sync; echo 3 | sudo tee /proc/sys/vm/drop_caches; sudo fstrim -av"
bpf_basic_command = "sudo python3 {} {} {}"

output_filename = "{}/sqlite_limit-{}_pcsize-{}_readratio-{}.{}"
...
for read_ratio in read_ratios:
    sqlite_output_filename = output_filename.format(result_dir, memory_limit / 10, cache_size, read_ratio, "sqlout")
    sqlite_command = sqlite_basic_command.format(memory_limit, target, cache_size, read_ratio, sqlite_output_filename)
