...
class ShardedLRUCache : public Cache {
 ...
 public:
  ...
  void ChangeCapacity(size_t capacity) override {
    const size_t per_shard = (capacity + (kNumShards - 1)) / kNumShards;
    for (int s = 0; s < kNumShards; s++) {
      shard_[s].ChangeCapacity(per_shard);
    }
  }
...
void MultiLengthLRUCache::Ref(LRUHandle* e) {
  for (uint64_t i = 1; i <= (uint64_t) e->value; ++i) {
    length_marks_[i]->value = (void*) i;
    length_marks_[i] = length_marks_[i]->next;
  }

  if ((uint64_t) e->value + 1 < length_marks_.size() && e == length_marks_[(uint64_t) e->value + 1]) {
    uint64_t i = uint64_t(e->value) + 1;
    length_marks_[i]->value = (void*) i;
    length_marks_[i] = length_marks_[i]->next;
  }
...
Cache::Handle* MultiLengthLRUCache::Insert(const Slice& key, uint32_t hash, void* value,
  ...
  for (uint64_t i = 1; i < length_marks_.size(); ++i) {
    length_marks_[i]->value = (void*) i;
    length_marks_[i] = length_marks_[i]->next;
  }
...
class ShardedMultiLengthLRUCache : public Cache {
 ...
 public:
  explicit ShardedMultiLengthLRUCache(size_t capacity, uint32_t num_marks) : last_id_(0),
    shard_{{num_marks}, {num_marks}, {num_marks}, {num_marks}, {num_marks}, {num_marks}, {num_marks}, {num_marks}, {num_marks}, {num_marks}, {num_marks}, {num_marks}, {num_marks}, {num_marks}, {num_marks}, {num_marks}} {
    const size_t per_shard = (capacity + (kNumShards - 1)) / kNumShards;
    for (int s = 0; s < kNumShards; s++) {
      shard_[s].SetCapacity(per_shard);
    }
...
Cache* NewLRUCacheLevelDB(size_t capacity) { return new ShardedLRUCache(capacity); }
Cache* NewMultiLengthLRUCacheLevelDB(size_t capacity, uint32_t num_marks) { return new ShardedMultiLengthLRUCache(capacity, num_marks); }
