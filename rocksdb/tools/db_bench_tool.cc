...
DEFINE_int64(num, 50000000, "Number of key/values to place in database");
...
DEFINE_int64(reads, 10000000, "Number of read operations to do.  "
...
DEFINE_int32(value_size, 80, "Size of each value in fixed distribution");
static unsigned int value_size = 80;
...
DEFINE_int64(memory_size, 0,
             "Number of bytes to use as total memory size for adaper, 0 = no adapter");

DEFINE_int32(cache_numshardbits, 0,
             "Number of shards for the block cache"
             " is 2 ** cache_numshardbits. Negative means use default settings."
             " This is applied only if FLAGS_cache_size is non-negative.");

DEFINE_double(cache_high_pri_pool_ratio, 0.5,
...
DEFINE_int64(compressed_cache_size, 8 << 20,
...
DEFINE_string(db, "/nvme/rocksdb/", "Use the db with the following name.");

DEFINE_string(output, "default.out", "Symbiosis output");

DEFINE_string(trace, "", "Symbiosis output");
...
void FormKey(int key_int, ROCKSDB_NAMESPACE::Slice key_slice) {
  std::string key_string = std::to_string(key_int);
  int i = 0;
  char* key_data = const_cast<char*>(key_slice.data());
  for (; i < FLAGS_key_size - int(key_string.length()); ++i) key_data[i] = '0';
  for (; i < FLAGS_key_size; ++i) key_data[i] = key_string[i - (FLAGS_key_size - int(key_string.length()))];
}

std::string form_key(int key_num) {
    std::string key_string = std::to_string(key_num);
    return std::string(FLAGS_key_size - key_string.length(), '0') + key_string;
}
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
...
#endif  // !ROCKSDB_LITE
  {
    ...
    Rebirth::Adapter::Init(FLAGS_memory_size, cache_->GetCapacity(), cache_, compressed_cache_);
    // Adapter::Init(FLAGS_memory_size, cache_->GetCapacity(), cache_);
  }
  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      } else if (name == "cachetest1") {
        method = &Benchmark::CacheTest1;
      } else if (name == "writesequential") {
        fresh_db = true;
 ...
 private:
  ...
  void WriteSequential(ThreadState* thread) {
    ...
    std::uniform_int_distribution<char> distribution(48, 126);
    std::mt19937 generator(210);

    for (int i = 0; i < FLAGS_num; ++i) {
      std::string value;
      int j = 0;
      for (; j < 44; ++j) value += "v";
      for (; j < FLAGS_value_size; ++j) value += distribution(generator);
      Status s = db_.db->Put(write_options_, form_key(i), value);
  ...
  void ReadTrace(ThreadState* thread) {
    ...
    ofstream output(FLAGS_output);
    ifstream trace(FLAGS_trace);

    uint64_t timer = FLAGS_env->NowMicros();
    for (int i = 0; i < FLAGS_reads; i++) {
      Status s;
      int key_int;
      trace >> key_int;
      std::string key = form_key(key_int);
      std::string value;
      s = db_.db->Get(options, key, &value);
      Rebirth::Adapter::instance->Record();
      Rebirth::Adapter::instance->ClearAfterRecord();
      if (s.ok()) {
        found++;
        bytes += key.size() + value.length() + user_timestamp_size_;
      } else {
      ...
      if ((i + 1) % (reads_ / 1000) == 0) {
        uint64_t temp = FLAGS_env->NowMicros();
        output << temp - timer << "\n";
        timer = temp;
      }
    }
...
// optimization, so skip the check of the max level.
  ...
  void CacheTest1(ThreadState* thread) {
    ...
    ofstream step_time_output("step/step_latency.txt");
