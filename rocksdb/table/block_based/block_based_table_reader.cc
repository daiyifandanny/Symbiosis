...
void dtrace_helper(BlockType bt, int start, uint64_t hit) {
  if (bt == BlockType::kData) {
    if (start == 1) {
      DTRACE_PROBE2(leveldb, bcache_start_probe, 0, 0);
    } else if (start == 0) {
      DTRACE_PROBE2(leveldb, bcache_end_probe, hit, 0);
    }    
  }  
  
  if (bt == BlockType::kIndex) {
    if (start == 1) {
      DTRACE_PROBE2(leveldb, fcache_start_probe, 0, 0);
    } else if (start == 0) {
      DTRACE_PROBE2(leveldb, fcache_end_probe, hit, 0);
    }    
  }
}
...
Status BlockBasedTable::MaybeReadBlockAndLoadToCache(
  ...
  // Adapter::instance->Record(reinterpret_cast<PosixRandomAccessFile*>(rep_->file.get()->file())->ino_, handle.offset(), handle.size(), 
  //   reinterpret_cast<Block*>(block_entry->GetValue())->ApproximateMemoryUsage(), is_cache_hit, kernel_hit);
  ...
  dtrace_helper(block_type, 0, (uint64_t) is_cache_hit);
...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
  ...
  if (!may_match) {
    ...
    for (iiter->Seek(key); iiter->Valid() && !done; iiter->Next()) {
      ...
      ino = reinterpret_cast<PosixRandomAccessFile*>(rep_->file.get()->file())->ino_;
      bno = reinterpret_cast<IndexBlockIter*>(iiter)->restart_index_;
      bno_max = reinterpret_cast<IndexBlockIter*>(iiter)->num_restarts_;
      kernel_size = v.handle.size();
