...
uint64_t read_gen_rebirth = WT_READGEN_START_VALUE;
...
__evict_lru_walk(WT_SESSION_IMPL *session)
{
    ...
    queue->evict_candidates = 20; // rebirth
...
__evict_walk_tree(WT_SESSION_IMPL *session, WT_EVICT_QUEUE *queue, u_int max_entries, u_int *slotp)
{
    ...
    for (evict = start, pages_already_queued = pages_queued = pages_seen = refs_walked = 0;
        ...
        if (page->read_gen == WT_READGEN_NOTSET)
            __wt_cache_read_gen_new(session, page, 1);
...
fast:
        ...
        // WT_ITEM key;
        // __wt_row_leaf_key(session, ref->page, ref->page->pg_row, &key, false);

        // FILE* f = fopen("wt_eviction_queue_trace.txt", "a");
...
__evict_page(WT_SESSION_IMPL *session, bool is_server)
{
    ...
    // FILE* f = fopen("wt_trace_eviction.txt", "a");
    // fprintf(f, "%.*s\n", (int) key.size, (char*) key.data);
    // fclose(f);
    ...
    // __wt_cache_read_gen_bump(session, ref->page); // rebirth
