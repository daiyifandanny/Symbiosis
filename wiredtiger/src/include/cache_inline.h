...
extern uint64_t read_gen_rebirth;
...
__wt_cache_read_gen_bump(WT_SESSION_IMPL *session, WT_PAGE *page)
{
    ...
    page->read_gen = ++read_gen_rebirth;
    return;
 ...
 */
static inline void
__wt_cache_read_gen_new(WT_SESSION_IMPL *session, WT_PAGE *page, int eviction_thread)
{
    WT_CACHE *cache;
