...
void dtraceHelper(int option, uint64_t cache_hit) {
    if (option == 0) {
        DTRACE_PROBE2(leveldb, fcache_start_probe, 0, 0);
    } else if (option == 1) {
        DTRACE_PROBE2(leveldb, fcache_end_probe, cache_hit, 0);
    }
}
...
)
{
    ...
    // __wt_block_buffer_to_addr(block, addr.addr, &logid, &offset, &size, &checksum);

    // FILE* f = fopen("wt_trace.txt", "a");
...
read:
            ...
            // ref->page->kernel_start = kernel_cache_start;
...
skip_evict:
            ...
            if (page->read_gen == WT_READGEN_NOTSET) {
                if (wont_need)
                    ...
                    __wt_cache_read_gen_new(session, page, 0);
             ...
             */
            dtraceHelper(1, cache_hit);
            app_cache_hit = cache_hit;
            app_cache_size = page->memory_footprint;
            app_cache_id = page->kernel_start;
