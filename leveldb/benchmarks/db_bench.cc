...
static int FLAGS_reads = 100000000;

// Number of concurrent threads to run.
static int FLAGS_threads = 1;

// Size of each value
static int FLAGS_value_size = 80;
...
static uint64_t FLAGS_cache_size = -1;

static int FLAGS_compressed_cache_size = -1;

static uint64_t FLAGS_memory_size = 0;

// Maximum number of files to keep open at the same time (use default if == 0)
static int FLAGS_open_files = 1048576;
...
static const char* FLAGS_db = "/nvme/leveldb-large/";

static const char* FLAGS_output = "step/step_latency.txt";

static const char* FLAGS_trace1 = "";

static const char* FLAGS_trace2 = "";

static const char* FLAGS_trace3 = "";

static const char* FLAGS_trace4 = "";

void CheckMemoryUsage2() {
...
public:
    ...
    Rep elapsed_time() {
        ...
        auto temp = Clock::now();
        auto counted_time = std::chrono::duration_cast<Units>(temp - start_point).count();
        start_point = temp;
        std::atomic_thread_fence(std::memory_order_relaxed);
 ...
 public:
  Benchmark()
      : cache_(FLAGS_cache_size >= 0 ? NewLRUCache(FLAGS_cache_size) : nullptr),
        compressed_cache_(FLAGS_compressed_cache_size >= 0 ? NewLRUCache(FLAGS_compressed_cache_size) : nullptr),
  ...
  void Run() {
    ...
    while (benchmarks != nullptr) {
      ...
      } else if (name == Slice("cachetest1")) {
        method = &Benchmark::CacheTest1;
      } else if (name == Slice("cachetest2")) {
        method = &Benchmark::CacheTest2;
      } else if (name == Slice("readtrace")) {
        method = &Benchmark::ReadTrace;
      } else if (name == Slice("writetrace")) {
        method = &Benchmark::WriteTrace;
 ...
 private:
  ...
  void RunBenchmark(int n, Slice name,
    ...
    for (int i = 0; i < n; i++) {
      ...
      arg[i].thread = new ThreadState(i, /*seed=*/1000);
  ...
  void Open() {
    ...
    options.compressed_cache = compressed_cache_;
    ...
    Adapter::Init(FLAGS_memory_size, cache_ == nullptr ? 8 << 20 : cache_->Capacity(), db_);
  }
  ...
  void ReadRandom(ThreadState* thread) {
    ...
    int found = 0;
    KeyBuffer key;
    uint64_t last_micro = g_env->NowMicros();
    int output_interval_ops = 10000;
    int next_report_ops = output_interval_ops;

    // preheat
    // for (int i = 0; i < reads_; i++) {
    //   const int k = thread->rand.Uniform(FLAGS_num);
    //   // const int k = (75 * i) % 80000000;
    //   key.Set(k);
    ...
    for (int i = 0; i < reads_; i++) {
      ...
      ++current_ops;
      if (current_ops >= next_report_ops) {
        uint64_t now_micro = g_env->NowMicros();
        fprintf(output_file, "%u %lu\n", current_ops, now_micro - last_micro);
        last_micro = now_micro;
        next_report_ops += output_interval_ops;
      }
    }
    // stat->PauseTimer(1);
    char msg[100];
  ...
  void ReadTrace(ThreadState* thread) {
    ReadOptions options;
    std::string value;
    KeyBuffer key;
    if (std::strstr(FLAGS_trace1, "hotspot90_10M_1.out") != nullptr) {
      FLAGS_trace2 = FLAGS_trace1;
    }
    ifstream inputs[4] = {ifstream(FLAGS_trace1), ifstream(FLAGS_trace2), ifstream(FLAGS_trace3), ifstream(FLAGS_trace4)};
    int k;    

    uint64_t found = 0;
    uint64_t last_micro = g_env->NowMicros(), simulation_start_micro;
    int output_interval_ops = 10000;
    int next_report_ops = output_interval_ops;
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (input) {
        for (input >> k; !!input; input >> k) {
          ...
          // if (current_ops == 10190000) {
          //   simulation_start_micro = g_env->NowMicros();
          // }
          // if (current_ops == 10199999) {
          //   db_->ChangeCacheCapacity(0);
          // }
          // if (current_ops == 12390000) {
          //   printf("mock simulation time %f s.\n", (g_env->NowMicros() - simulation_start_micro) / float(1000000));
          // }
        }      
      }
      // db_->ChangeCacheCapacity(0);
    ...
    char msg[100];
    std::snprintf(msg, sizeof(msg), "(%lu of %d found)", found, num_);
    thread->stats.AddMessage(msg);
  }

  void WriteTrace(ThreadState* thread) {
    ReadOptions options;
    std::string value;
    RandomGenerator write_values;
    KeyBuffer key;
    if (std::strstr(FLAGS_trace1, "hotspot90_10M_1.out") != nullptr) {
      FLAGS_trace2 = FLAGS_trace1;
    }
    ...
    ifstream w_inputs[4] = {ifstream(FLAGS_trace1), ifstream(FLAGS_trace2), ifstream(FLAGS_trace3), ifstream(FLAGS_trace4)};
    for (int i = 0; i < 4; ++i) {
      ifstream& input = w_inputs[i];
      if (input) {
        for (input >> k; !!input; input >> k) {
          key.Set(k);
          if (db_->Put(write_options_, key.slice(), write_values.Generate(FLAGS_value_size)).ok()) {
    ...
    uint64_t found = 0;
    ifstream r_inputs[4] = {ifstream(FLAGS_trace1), ifstream(FLAGS_trace2), ifstream(FLAGS_trace3), ifstream(FLAGS_trace4)};
    for (int i = 0; i < 4; ++i) {
      ifstream& input = r_inputs[i];
      if (input) {
        for (input >> k; !!input; input >> k) {
          key.Set(k);
          if (found % 10 < 2) {
  ...
  void CacheTest2(ThreadState* thread) {
    ReadOptions options;
    std::string value;
    int found = 0;
    KeyBuffer key;

    ofstream step_time_output(FLAGS_output);
    precise_stopwatch step_timer;
    
    SkewedWorkload sw = SkewedWorkload(0, FLAGS_num, FLAGS_num * 9 / 10, FLAGS_num, 0.9);
    SkewedWorkload sw2 = SkewedWorkload(0, FLAGS_num, FLAGS_num * 2 / 3, FLAGS_num, 0.667);

    for (int i = 0; i < reads_ / 2; i++) {
      const int k = sw.Target();
      key.Set(k);
      if (db_->Get(options, key.slice(), &value).ok()) {
        found++;
      }
      ...
      if ((i + 1) % (reads_ / 200) == 0) {
        auto diff = step_timer.elapsed_time();
        step_time_output << diff / 1000 << "\n";
      }
    }

    
    for (int i = 0; i < reads_ / 2; i++) {
      const int k = sw2.Target();
      key.Set(k);
      if (db_->Get(options, key.slice(), &value).ok()) {
        found++;
      }
  ...
  void CacheTest1(ThreadState* thread) {
    ReadOptions options;
    std::string value;
    int found = 0;
    KeyBuffer key;
    ...
    // CheckMemoryUsage();
    // db_->ChangeCacheCapacity(600000000);
    // malloc_trim(0);
    // pid_t pid = getpid();
    // string cmd = "echo 3 | sudo tee /proc/" + std::to_string(pid) + "/clear_refs";
    // system(cmd.c_str());
    // CheckMemoryUsage();
...
int main(int argc, char** argv) {
  ...
  auto ignore = system("sync; echo 3 | sudo tee /proc/sys/vm/drop_caches; sudo fstrim -av");
  (void) ignore;
  DTRACE_PROBE2(leveldb, fcache_start_probe, 0, 0);
  DTRACE_PROBE2(leveldb, fcache_end_probe, 0, 0);
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (leveldb::Slice(argv[i]).starts_with("--benchmarks=")) {
      ...
      FLAGS_key_prefix = n;
    } else if (sscanf(argv[i], "--cache_size=%lu%c", &un, &junk) == 1) {
      FLAGS_cache_size = un;
    } else if (sscanf(argv[i], "--compressed_cache_size=%d%c", &n, &junk) == 1) {
      FLAGS_compressed_cache_size = n;
    } else if (sscanf(argv[i], "--memory_size=%lu%c", &un, &junk) == 1) {
      FLAGS_memory_size = un;
      ...
      FLAGS_db = argv[i] + 5;
    } else if (strncmp(argv[i], "--trace1=", 9) == 0) {
      FLAGS_trace1 = argv[i] + 9;
    } else if (strncmp(argv[i], "--trace2=", 9) == 0) {
      FLAGS_trace2 = argv[i] + 9;
    } else if (strncmp(argv[i], "--trace3=", 9) == 0) {
      FLAGS_trace3 = argv[i] + 9;
    } else if (strncmp(argv[i], "--trace4=", 9) == 0) {
      FLAGS_trace4 = argv[i] + 9;
    } else if (strncmp(argv[i], "--output=", 9) == 0) {
      FLAGS_output = argv[i] + 9;
    } else {
  ...
  output_file = fopen(FLAGS_output, "w");
