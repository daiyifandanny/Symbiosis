...
function run_one_hotspot_pair() {
    ...
    do
        JOB_NAME="Job_sczipf-NumAccess_10M-Maxkey_${MAX_KEY}M-HotspotKeyPct-${HOTSPOT_KEY_PCT}_HotspotOpPct-${HOTSPOT_OP_PCT}"
        echo $JOB_NAME
        ...
        ls ${JOB_NAME}*
        mv ${JOB_NAME}*  ./req-trace-contzipf/
        mv Job_contzipf* ./req-trace-contzipf
    done
}
